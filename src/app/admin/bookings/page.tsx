    ...
    const loadBookings = async () => {
      ...
      try {
        ...
        if (supabase) {
          ...
          if (!error && data) {
            const supabaseBookings: Booking[] = data.map((b: any) => ({
              id: b.id,
              client: b.name,
              email: b.email,
              phone: b.phone,
              event: b.event_type || 'Not Specified',
              date: b.event_date || new Date().toISOString().split('T')[0],
              guests: parseInt(b.guest_count) || 50,
              status: b.status || 'New',
              value: b.budget || '$1,200',
              notes: b.special_requests || ''
            }));
            allBookings.push(...supabaseBookings);
          }
        }
      } catch (error) {
      ...
      // Also load from localStorage (backup/offline bookings)
      ...
      if (savedBookings) {
        ...
        // Merge with Supabase bookings (avoid duplicates by email+date)
        for (const localBooking of formatted) {
          const isDuplicate = allBookings.some(
            sb => sb.email === localBooking.email && sb.date === localBooking.date
          );
          if (!isDuplicate) {
      ...
      // Set all bookings (Supabase + localStorage combined)
