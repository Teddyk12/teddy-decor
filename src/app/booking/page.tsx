...
function BookingForm() {
  ...
  const [availableDates, setAvailableDates] = useState<{ date: string; available: boolean; slotsLeft: number; }[]>([]);
  ...
  const loadAvailability = async () => {
    try {
      ...
      // Load next 120 days of availability
      ...
      if (!error && data) {
        const formatted = data.map(d => ({
          date: d.date,
          available: d.is_available && d.current_bookings < d.max_bookings,
          slotsLeft: d.max_bookings - d.current_bookings
        }));
        setAvailableDates(formatted);
        console.log('📅 Loaded availability for', formatted.length, 'dates');
      }
    } catch (error) {
  ...
  const validateAvailability = async (selectedDate: string): Promise<boolean> => {
    setCheckingAvailability(true);

    try {
      const supabase = await getSupabase();

      // If Supabase not available, allow booking (fallback)
      if (!supabase) {
        console.warn('Supabase not available - allowing booking');
        return true;
      }
      ...
      if (error || !data) {
        // Date not in calendar - ask admin to add it
        toast.error('This date is not yet available for booking. Please contact us to check availability.', {
          duration: 5000
        });
        return false;
      }
      ...
      if (!data.is_available) {
        toast.error('Sorry, this date is not available for bookings. Please choose another date.', {
          duration: 5000
        });
        return false;
      }
      ...
      if (data.current_bookings >= data.max_bookings) {
        toast.error('Sorry, this date is fully booked. Please choose another date.', {
          duration: 5000,
          description: `We have ${data.max_bookings} bookings maximum per day`
        });
        return false;
      }

      // Date is available!
      const slotsLeft = data.max_bookings - data.current_bookings;
      console.log(`✅ Date available! ${slotsLeft} slot(s) remaining`);
      return true;

    } catch (error) {
      console.error('Availability check error:', error);
      // On error, allow booking (don't block customer)
      return true;
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.eventDate) {
        const isAvailable = await validateAvailability(formData.eventDate);
        if (!isAvailable) {
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-xl p-8 md:p-12">
          ...
          <div className="mb-10">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
              <Users className="w-6 h-6 mr-3 text-teddy-green" />
              Personal Information
            </h2>
          ...
          <div className="mb-10">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
              <Calendar className="w-6 h-6 mr-3 text-teddy-green" />
              Event Details
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
              ...
              <div>
                ...
                {formData.eventDate && (
                  <div className="mt-2">
                    {(() => {
                      const avail = availableDates.find(d => d.date === formData.eventDate);
                      if (avail) {
                        return avail.available ? (
                          <p className="text-sm text-green-600 flex items-center gap-1">
                            ✓ Date available ({avail.slotsLeft} slot{avail.slotsLeft !== 1 ? 's' : ''} remaining)
                          </p>
                        ) : (
                          <p className="text-sm text-red-600 flex items-center gap-1">
                            ✗ Date unavailable - please choose another date
                          </p>
                        );
                      }
          ...
          <div className="mb-10">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
              <MapPin className="w-6 h-6 mr-3 text-teddy-green" />
              Event Location
            </h2>
              ...
              <div className="flex space-x-4">
                <label className="flex items-center cursor-pointer">
                  <input
                    ...
                    onChange={handleChange}
                    className="w-4 h-4 text-teddy-green border-gray-300 focus:ring-rose-500"
                  />
          ...
          <div className="mb-10">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
              <Clock className="w-6 h-6 mr-3 text-teddy-green" />
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label className="flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-teddy-green-500 transition-colors">
                  <input
                    type="radio"
                    ...
                    className="w-4 h-4 text-teddy-green border-gray-300 focus:ring-rose-500"
                  />
                  <Video className="w-5 h-5 mx-3 text-teddy-green" />
                  <div>
                ...
                <label className="flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-teddy-green-500 transition-colors">
                  <input
                    ...
                    className="w-4 h-4 text-teddy-green border-gray-300 focus:ring-rose-500"
                  />
                  <Home className="w-5 h-5 mx-3 text-teddy-green" />
                  <div>
          ...
          <div className="mb-10">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
              <MessageSquare className="w-6 h-6 mr-3 text-teddy-green" />
          ...
          <div className="mt-10">
            <button
              ...
              className="w-full py-4 bg-gradient-to-r from-teddy-green to-teddy-gold text-white text-lg font-semibold rounded-full hover:shadow-xl hover:scale-105 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
    ...
    <Suspense fallback={
      <div className="min-h-screen bg-gradient-to-b from-white to-rose-50 py-12 flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-teddy-green-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
